...
//     // console.log('req', req);
//     next(); // <---- move to next middleware. done with this one. It goes to whatver comes next in app.use() but if the nexxt one does not match with the specified url, it will skip and move to the next one
// };
...
// app.get('/hello', //<---- if request type is /hello and is a GET type of request then and only then it will run
// //do something 
// )
...
app.use( (req, res, next) => { // handling error before the below error handler. handling 404 
    let err = new Error('Not Found');
    err.status = 404;
    next(err);
})
app.use( (err, req, res, next) => { // for when there is an explicit error
    res.status( err.status || 500); // error or generic 500-server error
    res.json({ // we can send a json 
        message: "Error!!",
        err: err
    });
})
